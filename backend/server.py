...
class Laboratory(BaseModel):
    ...
    participates_in_solidarity: bool = False  # مشترك في التكافل الاجتماعي
    neighborhood_id: str  # الحي
    user_id: Optional[str] = None  # معرف المستخدم المرتبط (من نوع laboratory)
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    updated_at: Optional[datetime] = None
    created_by: Optional[str] = None
...
class TakafulBenefit(BaseModel):
    ...
    original_amount: Optional[float] = None  # المبلغ الأصلي (للخصم)
    final_amount: Optional[float] = None  # المبلغ النهائي بعد الخصم
...
async def update_user_by_admin(
    ...
    old_email = (existing_user.get('email') or '').strip()
    ...
    new_phone = (update_data.get('phone') or '').strip()
    old_phone = (existing_user.get('phone') or '').strip()
...
async def create_takaful_benefit(
    ...
    benefit = TakafulBenefit(
        ...
        original_amount=benefit_data.original_amount,
        final_amount=benefit_data.final_amount,
