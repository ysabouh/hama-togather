...
import { 
  ...
  Gift, Phone, Mail, Package, MapPin, Download, RefreshCw, History, ChevronDown
} from 'lucide-react';
...
const DonationsManagement = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      {/* Transfer Donation Modal */}
      {showTransferModal && selectedDonation && (
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-gradient-to-r from-orange-600 to-red-600 text-white p-6 rounded-t-2xl">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold">نقل التبرع</h2>
                    <p className="text-orange-100 text-sm">اختر العائلة الجديدة المستفيدة</p>
                  </div>
                </div>
                <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <h3 className="text-sm font-bold text-gray-700 mb-2">العائلة الحالية:</h3>
                <div className="flex items-center gap-2">
                  <Users className="w-5 h-5 text-gray-600" />
                  <span className="text-gray-900 font-semibold">
                    {selectedDonation.family_name || 'غير محدد'}
                  </span>
                  {selectedDonation.family_number && (
                    <span className="text-gray-500 text-sm">({selectedDonation.family_number})</span>
                  )}
                </div>
              </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="text-sm text-yellow-800">
                    ...
                    <ul className="list-disc list-inside space-y-1">
                      <li>سيتم نقل التبرع للعائلة الجديدة</li>
                      <li>سيتم تحويل نوع التبرع من "قابل للنقل" إلى "ثابت"</li>
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const token = localStorage.getItem('token');
                    await axios.put(
                      `${API_URL}/donations/${selectedDonation.id}/transfer`,
                      { new_family_id: selectedFamilyForTransfer },
                      { headers: { Authorization: `Bearer ${token}` } }
                    );
                    toast.success('تم نقل التبرع بنجاح');
                    setShowTransferModal(false);
                    fetchData();
                  } catch (error) {
                    toast.error('فشل نقل التبرع');
                  }
                }}
                disabled={!selectedFamilyForTransfer}
